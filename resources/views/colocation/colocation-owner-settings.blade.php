<x-app-layout>
        ...
        <div class="flex items-center justify-between">
            <div>
                <div class="flex items-center gap-2 text-gray-400 mb-1">
                    <a href="{{ route('colocation.show', $colocation->id) }}" class="hover:text-orange-500 transition-colors text-xs font-bold uppercase tracking-widest flex items-center gap-1">
    ...
    <div class="py-10">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 space-y-8">
            ...
            <div class="bg-white shadow-[0_10px_40px_-15px_rgba(0,0,0,0.05)] sm:rounded-[2rem] border border-gray-100 overflow-hidden">
                <div class="p-8 border-b border-gray-50 flex items-center justify-between bg-[#FAF9F6]/30">
                    <div>
                        ...
                        <p class="text-sm text-gray-500">Ajoutez ou modifiez les catégories pour mieux organiser vos comptes.</p>
                    </div>
                ...
                <div class="p-8">
                    ...
                    <form action="{{ route('colocation.categories.store', $colocation->id) }}" method="POST" class="flex flex-wrap items-end gap-4 mb-10 bg-gray-50/50 p-6 rounded-3xl border border-dashed border-gray-200">
                        ...
                        <button type="submit" class="bg-[#1A1A1A] text-[#FAF9F6] px-8 py-3 rounded-2xl text-sm font-bold hover:bg-gray-800 transition-all shadow-md flex items-center gap-2">
                        ...
                        @foreach($colocation->categories as $category)
                            <div x-data="{ editing: false, name: '{{ $category->name }}' }" 
                                ...
                                <div class="flex items-center gap-4 flex-1">
                                    <div class="w-10 h-10 bg-gray-50 rounded-xl flex items-center justify-center text-gray-400 group-hover:bg-orange-50 group-hover:text-orange-500 transition-colors font-bold">
                                        {{ substr($category->name, 0, 1) }}
                                    </div>
                                    
                                    <div x-show="!editing" class="font-bold text-[#1A1A1A]">{{ $category->name }}</div>
                                    <input x-show="editing" x-model="name" type="text" 
                                           class="text-sm font-bold text-[#1A1A1A] border-none focus:ring-0 p-0 m-0 w-full bg-orange-50 rounded-md px-2 py-1">
                                </div>

                                <div class="flex items-center gap-2">
                                    <button @click="editing = !editing" x-text="editing ? 'Sauvegarder' : 'Modifier'"
                                            class="text-xs font-bold py-2 px-4 rounded-xl transition-all"
                                            :class="editing ? 'bg-orange-500 text-white shadow-md' : 'text-gray-400 hover:text-[#1A1A1A] hover:bg-gray-50'">
                                    </button>
                                    <button x-show="editing" @click="editing = false" class="text-xs font-bold text-gray-400 hover:text-red-500 px-2">
                                        Annuler
                                    </button>
                                </div>
                            </div>
            ...
            <div class="bg-blue-50/50 border border-blue-100 rounded-[2rem] p-6 flex items-start gap-4">
                ...
                <div>
                    ...
                    <p class="text-blue-700 text-xs mt-1 leading-relaxed">
                        Pour garantir l'intégrité de vos comptes, une catégorie ne peut pas être supprimée si elle contient déjà des dépenses. Vous pouvez cependant la renommer à tout moment.
                    </p>
                </div>
            </div>
        </div>
