<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center gap-4">
            <a href="{{ route('dashboard') }}" class="w-10 h-10 bg-white border border-gray-200 rounded-full flex items-center justify-center text-gray-500 hover:text-[#1A1A1A] hover:border-gray-300 transition-colors shadow-sm">
    ...
    <div x-data="{ showExpenseModal: false, showAddMemberModal: false }" class="py-10 relative">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 space-y-8">
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                ...
                <div class="bg-red-50 p-6 rounded-[2rem] shadow-[0_10px_40px_-15px_rgba(0,0,0,0.05)] border border-red-100 flex items-center gap-5 relative overflow-hidden">
                    <div class="w-14 h-14 bg-red-100 rounded-full flex items-center justify-center shrink-0 z-10">
                        <svg class="w-7 h-7 text-red-600" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13 17h8m0 0V9m0 8l-8-8-4 4-6-6"></path></svg>
                    </div>
                    <div class="z-10">
                    ...
                    <svg class="absolute -bottom-4 -right-4 w-32 h-32 text-red-100 opacity-50" fill="currentColor" viewBox="0 0 24 24"><path d="M12 2C6.48 2 2 6.48 2 12s4.48 10 10 10 10-4.48 10-10S17.52 2 12 2zm0 18c-4.41 0-8-3.59-8-8s3.59-8 8-8 8 3.59 8 8-3.59 8-8 8zm.31-8.86c-1.77-.45-2.34-.94-2.34-1.67 0-.84.79-1.43 2.1-1.43 1.38 0 1.9.66 1.94 1.64h1.71c-.05-1.34-.87-2.57-2.49-2.97V5H10.9v1.69c-1.51.32-2.72 1.3-2.72 2.81 0 1.79 1.49 2.69 3.66 3.21 1.95.46 2.34 1.15 2.34 1.87 0 .53-.39 1.64-2.25 1.64-1.74 0-2.1-.96-2.15-1.92H8.03c.05 1.68 1.15 2.87 2.87 3.25V19h2.36v-1.67c1.68-.34 2.88-1.48 2.88-3.04 0-2.23-1.85-2.92-3.83-3.15z"></path></svg>
                </div>
            </div>
            ...
            <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
                ...
                <div class="lg:col-span-2 space-y-6">
                    <div class="flex items-center justify-between bg-white p-4 rounded-3xl shadow-sm border border-gray-100">
                        ...
                        <button @click="showExpenseModal = true" class="bg-[#1A1A1A] text-[#FAF9F6] px-5 py-2.5 rounded-full text-sm font-bold hover:bg-gray-800 transition-all shadow-md flex items-center gap-2">
                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4v16m8-8H4"></path></svg>
                            Ajouter
                        </button>
                    </div>

                    <div class="bg-white p-5 rounded-[2rem] shadow-[0_10px_40px_-15px_rgba(0,0,0,0.05)] border border-gray-100 flex items-center justify-between hover:border-gray-200 transition-colors group cursor-pointer">
                        <div class="flex items-center gap-4">
                            ...
                            <div>
                                ...
                                <p class="text-sm text-gray-500 mt-0.5">Payé par <span class="font-semibold text-gray-700">Yassine</span> • Il y a 2 jours</p>
                            </div>
                        </div>
                    ...
                    <div class="bg-white p-5 rounded-[2rem] shadow-[0_10px_40px_-15px_rgba(0,0,0,0.05)] border border-gray-100 flex items-center justify-between hover:border-gray-200 transition-colors group cursor-pointer">
                        <div class="flex items-center gap-4">
                            ...
                            <div>
                                ...
                                <p class="text-sm text-gray-500 mt-0.5">Payé par <span class="font-semibold text-[#1A1A1A]">Moi</span> • Il y a 5 jours</p>
                            </div>
                        </div>
                        <div class="text-right">
                    ...
                    <div class="bg-white p-6 rounded-[2rem] shadow-[0_10px_40px_-15px_rgba(0,0,0,0.05)] border border-gray-100">
                        ...
                        <div class="space-y-4">
                            <div class="flex items-center justify-between bg-red-50 p-3 rounded-2xl border border-red-100">
                                <div class="flex items-center gap-3">
                                    <div class="w-8 h-8 rounded-full bg-red-200 flex items-center justify-center text-red-700 font-bold text-xs">Y</div>
                                    <p class="text-sm font-semibold text-red-700">Je dois à Yassine</p>
                                </div>
                                <span class="font-bold text-red-700">150 MAD</span>
                            </div>

                            <div class="flex items-center justify-between bg-green-50 p-3 rounded-2xl border border-green-100">
                                <div class="flex items-center gap-3">
                                    <div class="w-8 h-8 rounded-full bg-green-200 flex items-center justify-center text-green-700 font-bold text-xs">A</div>
                    ...
                    <div class="bg-white p-6 rounded-[2rem] shadow-[0_10px_40px_-15px_rgba(0,0,0,0.05)] border border-gray-100">
                        ...
                        <ul class="space-y-3">
                            <li class="flex items-center justify-between">
                                <div class="flex items-center gap-3">
                                    <div class="w-10 h-10 rounded-full bg-orange-100 flex items-center justify-center text-orange-600 font-bold text-sm">M</div>
                                    <div>
                                        <p class="text-sm font-bold text-[#1A1A1A]">Moi (Reda)</p>
                            ...
                            <li class="flex items-center justify-between">
                                <div class="flex items-center gap-3">
                                    <div class="w-10 h-10 rounded-full bg-gray-100 flex items-center justify-center text-gray-600 font-bold text-sm">Y</div>
                                    <div>
                                        <p class="text-sm font-bold text-[#1A1A1A]">Yassine</p>
                                        <p class="text-xs text-gray-400">Membre</p>
                                    </div>
                                </div>
                            </li>
                            <li class="flex items-center justify-between">
                                <div class="flex items-center gap-3">
                                    <div class="w-10 h-10 rounded-full bg-gray-100 flex items-center justify-center text-gray-600 font-bold text-sm">A</div>
                                    <div>
                                        <p class="text-sm font-bold text-[#1A1A1A]">Ahmed</p>
                                        <p class="text-xs text-gray-400">Membre</p>
                                    </div>
                                </div>
                            </li>
        ...
        <div x-show="showExpenseModal" style="display: none;" class="fixed inset-0 z-50 overflow-y-auto" aria-labelledby="modal-title" role="dialog" aria-modal="true">
            <div x-show="showExpenseModal" x-transition:enter="ease-out duration-300" x-transition:enter-start="opacity-0" x-transition:enter-end="opacity-100" x-transition:leave="ease-in duration-200" x-transition:leave-start="opacity-100" x-transition:leave-end="opacity-0" class="fixed inset-0 bg-gray-900/40 backdrop-blur-sm transition-opacity"></div>

            <div class="flex min-h-screen items-center justify-center p-4 text-center sm:p-0">
                <div x-show="showExpenseModal" @click.away="showExpenseModal = false" x-transition:enter="ease-out duration-300" x-transition:enter-start="opacity-0 translate-y-4 sm:translate-y-0 sm:scale-95" x-transition:enter-end="opacity-100 translate-y-0 sm:scale-100" x-transition:leave="ease-in duration-200" x-transition:leave-start="opacity-100 translate-y-0 sm:scale-100" x-transition:leave-end="opacity-0 translate-y-4 sm:translate-y-0 sm:scale-95" class="relative transform overflow-hidden rounded-[2rem] bg-white text-left shadow-2xl transition-all sm:my-8 sm:w-full sm:max-w-md border border-gray-100 p-8">
                    
                    <button @click="showExpenseModal = false" class="absolute top-6 right-6 text-gray-400 hover:text-[#1A1A1A] transition-colors">
                    ...
                    <form method="POST" action="#" class="space-y-5">
                        ...
                        <div>
                            ...
                            <input type="text" name="title" placeholder="ex: Courses de la semaine" class="w-full rounded-2xl border-gray-200 shadow-sm focus:border-orange-400 focus:ring focus:ring-orange-400/20 text-sm py-3 px-4 transition-all">
                        </div>

                        <div>
                            <label class="block text-sm font-semibold text-[#1A1A1A] mb-1.5">Montant (MAD)</label>
                            <div class="relative">
                                <input type="number" step="0.01" name="amount" placeholder="0.00" class="w-full rounded-2xl border-gray-200 shadow-sm focus:border-orange-400 focus:ring focus:ring-orange-400/20 text-lg font-bold py-3 pl-4 pr-16 transition-all">
                        ...
                        <div>
                            ...
                            <select name="paid_by" class="w-full rounded-2xl border-gray-200 shadow-sm focus:border-orange-400 focus:ring focus:ring-orange-400/20 text-sm py-3 px-4 transition-all bg-white">
                        ...
                        <div>
                            ...
                            <div class="space-y-2">
                                <label class="flex items-center p-3 border border-orange-200 bg-orange-50 rounded-2xl cursor-pointer hover:bg-orange-100 transition-colors">
                                    <input type="checkbox" name="split_with[]" value="me" checked class="w-4 h-4 text-orange-500 border-gray-300 rounded focus:ring-orange-500">
                                    <span class="ml-3 text-sm font-semibold text-[#1A1A1A]">Moi</span>
                                </label>
                                <label class="flex items-center p-3 border border-gray-200 rounded-2xl cursor-pointer hover:bg-gray-50 transition-colors">
                                    <input type="checkbox" name="split_with[]" value="yassine" checked class="w-4 h-4 text-orange-500 border-gray-300 rounded focus:ring-orange-500">
                                    <span class="ml-3 text-sm font-semibold text-gray-700">Yassine</span>
                                </label>
                                <label class="flex items-center p-3 border border-gray-200 rounded-2xl cursor-pointer hover:bg-gray-50 transition-colors">
                                    <input type="checkbox" name="split_with[]" value="ahmed" class="w-4 h-4 text-orange-500 border-gray-300 rounded focus:ring-orange-500">
                                    <span class="ml-3 text-sm font-semibold text-gray-700">Ahmed</span>
                                </label>
                            </div>
                            <p class="text-xs text-gray-400 mt-2 italic">Le montant sera divisé équitablement entre les personnes sélectionnées.</p>
                        </div>

                        <div class="mt-8 pt-4 border-t border-gray-100 flex items-center justify-end gap-3">
                            <button type="button" @click="showExpenseModal = false" class="px-5 py-2.5 text-sm font-semibold text-gray-500 hover:text-[#1A1A1A] hover:bg-gray-50 rounded-full transition-colors">
                                Annuler
                            </button>
